...
pygame.display.set_caption('Donut.C in .py from Jaival @2023')
...
while run:
    ...
    for j in range(0, 628, theta_spacing):  # 0 to 2pi
        for i in range(0, 628, phi_spacing):  #  0 to 2pi
            c = math.sin(i)
            d = math.cos(j)
            e = math.sin(A)
            f = math.sin(j)
            ...
            n = math.sin(B)
            t = c * h * g - f * e
            x = int(x_offset + 40 * D * (l * h * m - t * n))  # coordinate after rotation
            y = int(y_offset + 20 * D * (l * h * n + t * m))  #  coordinate after rotation
            o = int(x + columns * y)
    ...
    for i in range(len(b)):
        A += 0.00002  # for rotation
        B += 0.00001  # for rotation
        if i == 0 or i % columns:
